    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                setTimeout(() => setShowRegistrationSuccessAnimation(false), 3000); // Hide animation after 3 seconds
                // Clear the form after successful submission
                setProductName('');
                setProductPrice('');
                setProductUrl('');
                setProductDateCreated('');
            } else {
        ...
        <div className="main-listing container-fluid w-100 m-0 p-0 d-flex justify-content-center">
            {showRegistrationSuccessAnimation && (
                <div className="animation-overlay">
                    <Lottie options={defaultOptions} height={400} width={400}  animationData={loginsuccessanimation}/>
                </div>
            )}
            <div
                ...
                <i className="fa fa-cloud-upload text-white fs-3 align-content-center">
                    <span className="text-danger">Images go here</span>
                </i>
            </div>
            <div
                        ...
                        <form>
                            <div className="mb-3 w-50">
                                ...
                                <input type="text" className="form-control" id="productName" name="productName"
                                       placeholder="Enter product name" value={productName} onChange={handleChangeName}
                                       required/>
                            </div>
                            <div className="mb-3 w-50">
                                <label htmlFor="productPrice" className="form-label">Product Price:</label>
                                <input type="number" className="form-control" id="productPrice" name="productPrice"
                                       placeholder="Enter product price" min="14" value={productPrice}
                                       onChange={handleChangePrice} required/>
                            </div>
                            <div className="mb-3 w-50">
                                <label htmlFor="productUrl" className="form-label">Product URL:</label>
                                <input type="url" className="form-control" id="productUrl" value={productUrl}
                                       onChange={handleChangeUrl} name="productUrl" placeholder="Enter product URL"
                                       required/>
                            </div>
                            <div className="mb-3 w-50">
                                <label htmlFor="productDateCreated" className="form-label">Product Creation Date:</label>
                                <input type="date" className="form-control" id="productDateCreated"
                                       name="productDateCreated" value={productDateCreated}
                                       onChange={handleChangeDateCreated} required/>
                            </div>
